...
### D-23 — Authority order for the prior Commander Space prototype
- **Decision:** An external prior Commander Space prototype (`sigma-prototype.zip`, incl. `PRODUCT_REFERENCE.md` / `PRODUCT_REFERENCE_v1.1.md`) exists and was reviewed. The authority order is: **(1) current Owner-approved Product Truth and decisions → (2) the Company Brain as the canonical representation of that truth → (3) the prior prototype as Product Design Evidence / loss-prevention reference → (4) historical documents.** The prototype **must not drive** the Company Brain; the Company Brain must drive the **next** prototype. The prototype was used only to answer *"did we already think through a business capability that is still compatible with today's direction and would otherwise be accidentally lost?"* — never *"what must the current product be?"*
- **Rationale:** The prototype is unfinished design exploration, not an owner-approved specification. Treating it as a source of truth would let unvalidated UX (exact recommendation-contract wording, an "Executive Brief" screen, a fixed widget catalog, a specific decision taxonomy, specific personas, the old two-workspace IA) silently become canonical.
- **Implications:** Specific prototype UX — the exact Recommendation Contract structure, Executive Brief, Awareness/Decision-mode framing, the widget catalog, the Tactical/Operational/Strategic/Informational decision taxonomy, its 5 personas, its routes/screens, its interaction mechanics — is **retained only as design evidence**, not promoted to current truth. Only capabilities independently confirmed by the Owner (D-24) are captured as current.
- **Affected:** Commander Space README; CURRENT-STATE.

### D-24 — Commander Space capability model; Planning and Control as confirmed business territories
- **Decision:** Commander Space's key business question is **"What requires my attention?"**, determined by `Role × Responsibility × Operational Context`, with conceptual progression **Attention → Understand → Decide/Approve → Act → Follow-up/Remember** (not mandatory IA). Confirmed current business capabilities: **Attention** (ranking ≠ recommendation), **Notifications/Alerts** (support Attention but are not automatically Attention items — reduce noise), **Situation Assessment**, **native/domain capability composition** by role/context (not a fixed dashboard, not a generic widget wall, not a replacement for professional systems), **Decision Support** (situation/meaning/evidence; never a forced decision, recommendation, or alternatives), **Approvals** (what requires/is waiting for my approval — aggregation, context, evidence, missing-condition indication, in-Zira completion where authority permits, follow-up), **Directives/Follow-up**, **Knowledge Continuity**, **Evidence & Trust**. **Commander Space boundary:** it does not own every workflow that generates attention — Planning, Control, Operations Store, Geography, and Context & Meaning can each generate/expose something it surfaces, without it absorbing that workflow or information ownership.
  Separately, **Planning** and **Control** are confirmed additional Zira business territories, distinct from each other and from Commander Space: **Planning** — *what are we planning, and how do we prepare/manage it?* (Operations Management / GANTTIT is important prior learning; first Vertical Slice remains unvalidated pending the recorded workflow-discovery TODO). **Control** — *what is happening now, and how do we manage/respond to it?* — exists **today** through the current **Event / Combat Management** capability (Operational Event managed Initial Report → Accumulating Information → Decisions/Directives/Involved Actors → Handling → Closure; current pain is manual/fragmented, insufficient real-time enrichment, SOP outside the event workflow; direction is enrichment, reduced friction, SOP-embedding, standardization, closed-events-as-organizational-data — **analytics explicitly not MVP**). **Planning/Control final product packaging (separate modules, one module with modes, or another composition) is intentionally not decided.**
- **Rationale:** Closes the material gaps identified by the Product Landscape Review and the Content Coverage Test (missing Control/Event-Management territory, mixed Planning/Control framing in Operations Management, undocumented Commander Space capability inventory) using Owner-confirmed truth — not the prototype.
- **Implications:** Commander Space README, CURRENT-STATE, and PR-001 updated to reflect the capability model and the two new territories. No module architecture is chosen. No delivery status is implied (GREENFIELD baseline unchanged, D-20). Operations Management's discovery TODO (first Vertical Slice hypothesis) is unaffected and remains open.
- **Affected:** Commander Space README; CURRENT-STATE; PR-001; Operations Management (cross-referenced, not edited).
