...
**Operations Management** — the **module inside Zira** for creating, editing and managing Operations, Activities and the operational lifecycle. It **uses Operations Store as its canonical backbone**. Current direction is intentionally simpler than the former standalone GANTTIT (Flat / basic Gantt / Operation Page / create-edit Operations & Activities are **candidate** product responses, not settled slice content). See [`02-DOMAINS/Operations/Product/Operations-Management.md`](02-DOMAINS/Operations/Product/Operations-Management.md). **The first Vertical Slice is not yet defined** — the 24–72h control-user JTBD is a **hypothesis pending workflow discovery** (see that doc's "Next Product Step"). *(Distinct from Operations Store; not part of Commander Space.)*
...
**Commander Space** — **Zira's role-based personal operational workspace.** Its key business question is **"What requires my attention?"** — determining what from the operational world materially requires this user, based on `Role × Responsibility × Operational Context`. It serves commanders **and other operational roles** ("Commander" describes the *decision-oriented* nature of the experience, not an exclusive user population). Its experience model is **Attention → Understand → Decide/Approve → Act → Follow-up/Remember** (conceptual, not mandatory IA; a user may enter at the relevant level). **Personalization changes attention, context, prioritization and available actions — not the underlying facts**, which remain shared and source-owned. Current confirmed business capabilities: **Attention** (ranking ≠ recommendation); **Notifications/Alerts** (support Attention but are not automatically Attention items — reduce noise); **Situation Assessment**; **native/domain capability composition** by role/context (not a fixed dashboard); **Decision Support** (situation/meaning/evidence, never a forced decision or recommendation); **Approvals** (what requires/is waiting for my approval, aggregated with context and evidence); **Directives/Follow-up**; **Knowledge Continuity**; **Evidence & Trust**. **Commander Space does not own the underlying workflows that generate attention** — Planning, Control, Operations Store, Geography, and Context & Meaning can each generate or expose something Commander Space surfaces, without Commander Space absorbing that workflow or information ownership. **My Space** is **absorbed** into Commander Space (no longer a separate product concept); **Headquarters Workspace** is **not** a current canonical module — its validated **Operational Awareness** learning ("what changed in the operational picture?") remains a **reusable experience pattern**. See [`03-EXPERIENCE/Commander-Space/README.md`](03-EXPERIENCE/Commander-Space/README.md). *(Current product/UX direction, not evidence of implementation. A prior prototype was reviewed as design evidence only — it is not the source of this direction and its specific UX is not canonical; see [D-23](05-DECISIONS/DECISION-LOG.md).)*

**Planning** — *what are we planning to happen, and how do we prepare/manage it?* A business territory being brought into **Zira** (operations, operational plans, activities, timelines, dependencies, preparation, Gantt/planning experiences). **Operations Management / GANTTIT provides important prior learning** for this territory (see the Operations Management entry above). The first Vertical Slice remains intentionally unvalidated pending the already-recorded workflow discovery TODO.

**Control** — *what is happening now, and how do we manage/respond to it?* Exists **today** inside the Zira world through the current **Event / Combat Management** capability. The Operational Event is managed from **Initial Report → Accumulating Information → Decisions/Directives/Involved Actors → Handling → Closure**. Current pain: largely manual/fragmented; commanders/control roles collect relevant information themselves; insufficient real-time enrichment; SOP often exists outside the event workflow. Product direction: reduce response time; enrich the event with relevant information; reduce information-gathering friction; embed SOP/organizational knowledge into the event flow; standardize fields/process/timeline; turn closed events into accumulated organizational data (**analytics is explicitly not part of MVP**).

**Planning / Control packaging is intentionally undecided** — these are clear business territories, not yet resolved into separate modules, one module with modes, or another composition ([D-24](05-DECISIONS/DECISION-LOG.md)).
...
All major current Sigma/Zira capability areas **require setup from zero**: Operations Store, Operations Management (Planning), Geography / Spatial Intelligence, Context & Meaning, Zira's treatment of Control (see below), and Zira's product modules (Commander Space, Operations Management). No code, test, deployment, pilot, or live Sigma/Zira evidence exists in the repository. Treat every Sigma/Zira capability as not-yet-built until implementation evidence appears. *(Note: Event/Combat Management as an operational capability/system already exists today outside Sigma, in the same way GANTTIT existed prior to Operations Management — its integration/reimagining as a Zira Control territory is what is GREENFIELD.)*
...
- **Direction (current product/UX intent):** Commander Space as Zira's role-based, decision-first personal workspace (Attention → Understand → Decide/Approve → Act → Follow-up/Remember) with the confirmed capability set (Attention, Notifications, Situation Assessment, native/domain composition, Decision Support, Approvals, Directives/Follow-up, Knowledge Continuity, Evidence & Trust — exact UX not locked); Operations Management simpler experience (Flat/Gantt/Operation Page); Control as a business territory served today by Event/Combat Management, with a stated pain/opportunity direction (enrichment, reduced friction, SOP embedding, standardization, closed-events-as-org-data); Planning as a business territory being brought into Zira; Planning/Control packaging undecided; Geography Text↔Map (P1).
...
5. Discovery/research (RS-001/RS-003) remains **unwritten (shells)**. *(DOC-001, OM-001 and PR-001 have since been rebuilt — this item previously listed them in error.)*
6. **Control / Event Management** — the business capability, pain, and direction are confirmed, but its detailed product design (fields, standardized process, SOP-embedding mechanics, closed-event data model) is not yet defined.
7. **Planning** — brought into Zira as a confirmed territory, but its first Vertical Slice remains unvalidated pending workflow discovery (see the Operations Management "Next Product Step"), and its **packaging relative to Control is undecided.**

> **Note for maintainers:** `GLOSSARY.md` and `DOCUMENT-REGISTRY.md` reflect the Sigma-as-ecosystem/Zira model, the Operational Signal resolution, and the GANTTIT → Operations Management decision as of the Phase 3C control-plane reconciliation. They have **not** been updated for the Commander Space capability model or the Planning/Control territories captured here and in the Decision Log (D-22–D-24) — only touch them if a specific statement becomes factually incorrect, not for general expansion.
